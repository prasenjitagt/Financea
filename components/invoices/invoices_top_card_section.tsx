...
export default function InvoicesTopCardsSection({ invoiceStats }: PropType) {
  ...
  const [amountDetails, setAmountDetails] =
    useState<AmountAndCurrencyType | null>(null);
  const [outstandingAmountDetails, setOutstandingAmountDetails] =
    useState<AmountAndCurrencyType | null>(null);
  ...
  useEffect(() => {
    async function fetchConvertedAmount() {
      try {
        ...
        const outstandingConvertedCurrency = await convertCurrency(
          totalOutstandingAmountINRClients,
          totalOutstandingAmountUSDClients,
          currency
        );
  ...
  return (
    <div className="flex justify-between  gap-4 mb-[38px]">
      <Card className="flex justify-center w-[300px] h-[140px] bg-[#FCFDFF]">
        <CardContent className="">
          <HeaderInfoCard
            mainText={"Total Invoices"}
            count={`${totalInvoices}`}
          />
        ...
        <CardContent className="">
          <HeaderInfoCard
            ...
            count={
              amountDetails === null
                ? "loading..."
                : `${amountDetails.currencySymbol} ${amountDetails.totalAmount}`
            }
          />
      ...
      <Card className="flex justify-center w-[300px] h-[140px] bg-[#FCFDFF]">
        <CardContent className="">
          <HeaderInfoCard
            mainText={"Outstanding Invs."}
            count={`${totalOutstandingInvoices}`}
          />
          <div className="flex items-end">
            <p className="text-muted-foreground text-[14px]">Last 30 Days</p>
          </div>
      ...
      <Card className="flex justify-center w-[300px]  h-[140px] bg-[#FCFDFF]">
        <CardContent className="   ">
          <HeaderInfoCard
            mainText="Total Amount"
            count={
              outstandingAmountDetails === null
                ? "loading..."
                : `${outstandingAmountDetails.currencySymbol} ${outstandingAmountDetails.totalAmount}`
            }
          />
